...
public var counter = 0;							// number of tail elements
private var lastChain : GameObject = null;	// last tail in the end of snake chain
private var moveSpeed = 3;					// movement speed of snake (gamespeed)
...
function FireWorks () {
		...
		var obj3 : Rigidbody = Instantiate(object, Vector3(Random.Range(1, 10), 1, Random.Range(1, 1)), Quaternion(0,0,0,0));
		obj.gameObject.renderer.material.color = Color(Random.value, Random.value, Random.value);
}
function Madness(){
		var obj : Rigidbody = Instantiate(object, Vector3(Random.Range(2, 1), 1, Random.Range(1, 1)), Quaternion(0,0,0,0));
		var obj2 : Rigidbody = Instantiate(object, Vector3(Random.Range(2, 10), 1, Random.Range(1, 1)), Quaternion(0,0,0,0));
		var obj3 : Rigidbody = Instantiate(object, Vector3(Random.Range(2, 15), 1, Random.Range(1, 1)), Quaternion(0,0,0,0));
		var obj4 : Rigidbody = Instantiate(object, Vector3(Random.Range(2, 15), 1, Random.Range(1, 1)), Quaternion(0,0,0,0));
		var obj5 : Rigidbody = Instantiate(object, Vector3(Random.Range(2, 15), 1, Random.Range(1, 1)), Quaternion(0,0,0,0));
		var obj6 : Rigidbody = Instantiate(object, Vector3(Random.Range(2, 15), 1, Random.Range(1, 1)), Quaternion(0,0,0,0));
		obj.gameObject.renderer.material.color = Color(Random.value, Random.value, Random.value);
}
...
function OnCollisionEnter(c : Collision) {
	...
	if(c.gameObject.CompareTag("Respawn")) {
		Application.LoadLevel("CubeSprint2");
	}
...
function Update () {
	...
	if(Input.acceleration.sqrMagnitude != 0) {
		...
		transform.rotation *= Quaternion.Euler(0, -Input.acceleration.y*Time.deltaTime*turnSpeed, 1);
	} else { // pc
		if(dx != 0) {
